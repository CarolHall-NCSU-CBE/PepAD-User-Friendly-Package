    ...
    subroutine read_restraints_input(input, signal)
        ...
        select case(signal)
            ...
            case (3)
				...
				write(*,*) "Grouped positional constraint AA pool: ", NMR_AA_pool(1)
		...
		do j=1, gnum
			do k=1, group(i,j)%cnum1
				if(group_para(i,j)%dielecons1(k)<=0.1) then
					open(10, file="error.txt", access="append")
						write(10,*) group(i,j)%gtype, i, j, "cluster 1 has wrong force field parameter", group_para(i,j)%dielecons1(k), "in the LIB!"
			...
			do k=1, group(i,j)%cnum2
				if(group_para(i,j)%dielecons2(k)<=0.1) then
					open(10, file="error.txt", access="append")
						write(10,*) group(i,j)%gtype, i, j, "cluster 2 has wrong force field parameter", group_para(i,j)%dielecons2(k), "in the LIB!"
						write(10,*) "Please check whether the atom type of PDB file matches the atom type of Force Field LIB or not!"
					close(10)
					stop
				endif
			...
			do k=1, group(i,j)%cnum3
				if(group_para(i,j)%dielecons3(k)<=0.1) then
					open(10, file="error.txt", access="append")
						write(10,*) group(i,j)%gtype, i, j, "cluster 3 has wrong force field parameter", group_para(i,j)%dielecons3(k), "in the LIB!"
						write(10,*) "Please check whether the atom type of PDB file matches the atom type of Force Field LIB or not!"
					close(10)
					stop
				endif
