...
module pdbfile
	...
	contains
	subroutine analyzepdb(user_n_aa,user_chains)
	implicit none
	integer, intent(in), optional	:: user_n_aa, user_chains
	integer						:: anum, status, ic, previous_ic
	integer						:: nres, candidate, repeat_count, i, j, chain_id
	integer						:: chain_hydrophobic, chain_polar, chain_charged, chain_other
	real						:: x, y, z
	...
	if(present(user_n_aa).neqv.present(user_chains)) then
		stop "ERROR: N_AA and N_CHAINS must be supplied together."
	endif

	if(present(user_n_aa)) then
		! Use the chain count supplied by the user. The complete residue count
		! per chain is obtained from the PDB because caps are not included in N_AA.
		if(user_n_aa.le.0.or.user_chains.le.0) then
			stop "ERROR: N_AA and N_CHAINS must be positive."
		endif
		if(mod(nres,user_chains).ne.0) then
			stop "ERROR: the PDB residue count is not divisible by N_CHAINS."
		endif
		gnum=nres/user_chains
		...
		if(.not.pattern_found) then
			stop "ERROR: PDB chains are different; supply N_AA and N_CHAINS."
		endif
	endif
	...
	if (present(user_n_aa)) then
		if (amino_acid_residue_count /= user_n_aa) then
			stop 'ERROR: N_AA does not match the number of amino acids in PDB chain 1.'
		endif
		do chain_id=2,repeated_unit
			call count_amino_acid_categories( &
				residue_names((chain_id-1)*gnum+1:chain_id*gnum), &
				chain_hydrophobic,chain_polar,chain_charged,chain_other)
			if (chain_hydrophobic+chain_polar+chain_charged+chain_other /= user_n_aa) then
				write(*,'(A,I0,A)') 'ERROR: N_AA does not match the number of amino acids in PDB chain ', &
					chain_id,'.'
				stop
			endif
		enddo
	...
	if (.not.any(composition_min_set) .and. &
		...
		if (.not.composition_max_set(1)) fpho_max=amino_acid_residue_count
		if (.not.composition_min_set(2)) fpol_min=0
		if (.not.composition_max_set(2)) fpol_max=amino_acid_residue_count
		if (.not.composition_min_set(3)) fchg_min=0
		if (.not.composition_max_set(3)) fchg_max=amino_acid_residue_count
		if (.not.composition_min_set(4)) foth_min=0
		if (.not.composition_max_set(4)) foth_max=amino_acid_residue_count
	endif
...
module input
	...
	integer :: unit, ios, line_number, equals_at, expected_n_aa, expected_chains
	integer :: clock_count
	...
	do
		...
		case('N_AA')
			read(value,*,iostat=ios) expected_n_aa
	...
	! N_AA and N_CHAINS are optional, but they must be used together.
	if (expected_n_aa == -1 .and. expected_chains == -1) then
		call analyzepdb
	elseif (expected_n_aa > 0 .and. expected_chains > 0) then
		call analyzepdb(expected_n_aa,expected_chains)
	else
	...
	if (fpho_min < 0 .or. fpho_min > amino_acid_residue_count .or. &
		fpho_max < 0 .or. fpho_max > amino_acid_residue_count .or. &
		fpol_min < 0 .or. fpol_min > amino_acid_residue_count .or. &
		fpol_max < 0 .or. fpol_max > amino_acid_residue_count .or. &
		fchg_min < 0 .or. fchg_min > amino_acid_residue_count .or. &
		fchg_max < 0 .or. fchg_max > amino_acid_residue_count .or. &
		foth_min < 0 .or. foth_min > amino_acid_residue_count .or. &
		foth_max < 0 .or. foth_max > amino_acid_residue_count) then
	...
	do i=1,n_morethan
		if (aa_morethan(i)%min < 0 .or. &
			aa_morethan(i)%min > amino_acid_residue_count) then
			write(*,*) 'ERROR: invalid minimum for ',aa_morethan(i)%gtype
			stop
		endif
		aa_morethan(i)%max=amino_acid_residue_count
	enddo

	do j=1,n_restrictions
		if (aa_restrictions(j)%max < 0 .or. &
			aa_restrictions(j)%max > amino_acid_residue_count) then
	...
	write(6,'(1X,A,T38,"= ",I0)') 'Number of amino acids', &
		amino_acid_residue_count
	write(6,'(1X,A,T38,"= ",I0)') 'Number of residues (including caps)',gnum
	write(6,'(1X,A,T38,"= ",I0)') 'Number of chains',repeated_unit
	...
	if (n_morethan == 0 .and. n_restrictions == 0) then
		...
		do i=1,n_morethan
			...
			max_value=amino_acid_residue_count
			do j=1,n_restrictions
				if (aa_morethan(i)%gtype == aa_restrictions(j)%gtype) then
	...
	write(6,'(1X,A,T38,"= ",I0)') 'Number of caps',cap_site_count

	write(6,'(1X,A,T38,"= ",I0)') 'Number of single-site constraints', &
		void_site_num-cap_site_count
	if (void_site_num > cap_site_count) then
		write(6,'(1X,A,T38,"= ",*(I0,1X))') &
			'Single-site amino-acid positions', &
			(amino_acid_site_from_chain_site(void_site(i)), &
			i=cap_site_count+1,void_site_num)
	else
		write(6,'(1X,A,T38,"= ",A)') &
			'Single-site amino-acid positions','none'
	endif
	write(6,'(1X,A,T38,"= ",I0)') 'Number of grouped-site constraints',nmr_site_num
	if (nmr_site_num > 0) then
		write(6,'(1X,A,T38,"= ",*(I0,1X))') &
			'Grouped-site amino-acid positions', &
			(amino_acid_site_from_chain_site(nmr_site_ID(i)),i=1,nmr_site_num)
	else
		write(6,'(1X,A,T38,"= ",A)') &
			'Grouped-site amino-acid positions','none'
	endif
	...
	integer function amino_acid_site_from_chain_site(chain_site)
		implicit none
		integer, intent(in) :: chain_site
		integer :: cap, cap_site_count

		amino_acid_site_from_chain_site=chain_site
		cap_site_count=gnum-amino_acid_residue_count
		do cap=1,cap_site_count
			if (void_site(cap) == chain_site) &
				call input_error('cap residue has no amino-acid position',0)
			if (void_site(cap) < chain_site) &
				amino_acid_site_from_chain_site=amino_acid_site_from_chain_site-1
		enddo
	end function amino_acid_site_from_chain_site
...
subroutine check_help
	...
	write(*,'(A)') '======================================================================'
	write(*,'(A)') 'Usage:'
	write(*,'(A)') '  PepAD                 Run a design using input_v1.42.txt'
	...
	write(*,'(A)') '  N_AA                          Integer; amino acids per chain, excluding caps'
	...
	write(*,'(A)') '                                (default 0)'
    write(*,'(A)') '  NOTE:'
	write(*,'(A)') '  If N_AA and N_CHAINS are omitted, both values are'
	write(*,'(A)') '  determined automatically from the PDB file.'
	write(*,'(A)') '  If either N_AA or N_CHAINS is provided, both must be provided.'
	...
	write(*,'(A)') '  An omitted MIN becomes 0; an omitted MAX becomes N_AA.'
